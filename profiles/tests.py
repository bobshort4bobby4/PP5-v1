...
class TestProfileViews(TestCase):
    ...
    def test_order_history_uses_correct_template(self):
        ...
        session['trade_flag'] = False
        session.save()
        response = self.client.get(reverse('profiles:order_history', kwargs={"order_number":ord.order_number}))
        messages = list(get_messages(response.wsgi_request))     
        self.assertEqual(len(messages), 1)
        self.assertEqual(str(messages[0]), f'This is a past confirmation for order number {ord.order_number}. '
